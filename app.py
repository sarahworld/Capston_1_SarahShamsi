...
def search():
    ...
    if form.validate_on_submit():
        ...
        model="gpt-3.5-turbo",
        messages=[
            {"role": "user", "content": f"What is the country of origin of {product_name}?
             Give one word answer.Is it vegetarian or not? Give your confidence percentage? Give two words answer."}]
        )
        # print(completion.choices[0].message)
        description= completion.choices[0].message
        print(description)

        return render_template('result.html',page_title="result", product_name=product_name, description=description)
